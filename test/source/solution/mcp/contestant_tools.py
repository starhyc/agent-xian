...
def register_tools(*, register_tool: Callable[..., Callable], object_schema: Callable[..., dict[str, Any]]) -> None:
    ...
    @register_tool(
        ...
        description="Demo-only mock MCP-style tool. Returns a fixed mock order lookup result.",
    ...
    @register_tool(
        ...
        description="Demo-only mock MCP-style tool. Returns a fixed mock policy check result.",
        input_schema=object_schema(
            {
                "payload": {
                    "type": "string",
    ...
    def zip_extract(zip_path: str, extract_dir: str) -> str:
        ...
        output_dir = Path(extract_dir).resolve()
        ...
        with zipfile.ZipFile(zip_file, "r") as zf:
            for member in zf.infolist():
                ...
                if not member_path.is_relative_to(output_dir):
                    return json.dumps(
                        {"error": f"Unsafe zip member path: {member.filename}"},
                        ensure_ascii=False,
                    )
        ...
        input_schema=object_schema(
            {
                ...
                "max_body_chars": {
                    "type": "integer",
                    "description": "Maximum response body characters to return. Use 0 for no truncation.",
                    "default": 10000,
                },
            },
            ["url"],
            ...
            return json.dumps(
                {
                    ...
                    "body": body,
                    "truncated": max_body_chars > 0 and len(response.text) > max_body_chars,
        ...
        input_schema=object_schema(
            {
                ...
                "stdin": {
                    "type": "string",
                    "description": "Optional standard input passed to the Java program.",
                    "default": "",
                },
            },
            ["java_file"],
